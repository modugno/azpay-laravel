...
use AzPayLaravel\AzPay\SDK\XML_Requests;
use Exception;
...
class AZPay_SDK {
    ...
    const PAGSEGURO_CHECKOUT = 'pagseguro_checkout';
    const PAYPAL = 'paypal';
    ...
    /**
     * Return a XML obfuscated
     * Use to log
     *
     * eg: $xml = $azpay->sale()->getObfuscatedXML();
     * eg: $xml = $azpay->getObfuscatedXML(); // after execute action
     *
     * @return XMLObject
     */
    public function getObfuscatedXML() {

        $obfuscated = clone $this;
        $obfuscated->config_card_payments['cardNumber'] = preg_replace('/[0-9]/', '*', substr($obfuscated->config_card_payments['cardNumber'], 0, -4)) . substr($obfuscated->config_card_payments['cardNumber'], -4);
        $obfuscated->config_card_payments['cardSecurityCode'] = preg_replace('/[0-9]/', '*', $obfuscated->config_card_payments['cardSecurityCode']);

        switch ($this->action) {
            case self::AUTHORIZE:
                return $obfuscated->authorize()->getXml();
            case self::SALE:
            ...
            case self::PAGSEGURO:
                return $obfuscated->pagseguro()->getXml();
            case self::PAGSEGURO_CHECKOUT:
                return $obfuscated->pagseguro_checkout()->getXml();
            case self::PAYPAL:
                return $obfuscated->paypal()->getXml();
            case self::ONLINE_DEBIT:
                return $obfuscated->online_debit()->getXml();
            case self::REBILL:
                return $obfuscated->rebill()->getXml();
            default:
                return null;
        }
    ...
    public function getCurlResponse() {
        return clone $this->curl_response;
    }
    ...
    public function getCurlResponseMeta() {
        return clone $this->curl_response_meta;
    }
    ...
    public function getCurlErrorCode() {
        return clone $this->curl_error_code;
    }
    ...
    public function pagseguro_checkout() {
        ...
        $this->action = 'pagseguro_checkout';
